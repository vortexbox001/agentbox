...
- **Viewer (management UI).** New Runs area: run list (agent, time, status, model, cost, attempts) with filters; run page built from the design-system `run-detail` template (`ui/design-system/templates/run-detail/`, entry `RunDetail.dc.html`) with tabs — **Conversation** (threaded, collapsible tool calls/results, diffs as diffs, per-turn tokens/cost, search), **Context**, **Report**, **Files**. A **Compare** action diffs two runs' `context.json`. Post-run only.
