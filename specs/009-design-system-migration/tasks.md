...
- [ ] T001 Establish the test baseline: run `cd ui && ../.venv/bin/python -m pytest -q` and record which suites are red (expect `tests/test_design_system_docs.py` red — it references deleted Archon files) versus green, so later regressions are attributable.
