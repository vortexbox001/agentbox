...
| GET | `/runs/{run_id}` | `runs/detail.html` | single page (design-system run-detail specimen): a transcript timeline with a Readable/Raw-log toggle beside a configuration rail carrying context, report, and output artifacts; FR-019. |
...
## Sections (FR-019, FR-019a, FR-020, FR-021, FR-022)

Composed as ONE page per the design-system run-detail specimen (amended from the original
four-tab shape): a transcript timeline in the main column with a Readable/Raw-log segmented
toggle, beside a configuration rail. The four concerns map onto that single page:

- **Conversation** (main column, Readable view) — threaded, chat-style history rendered from
  `events.jsonl`: turns in order, collapsible tool calls + results, file edits as rendered diffs,
  per-turn tokens + cost, in-page search. **Identical shape for every harness** (SC-002). Missing
  tool results are marked as missing, not hidden (FR-021). Absent `events.jsonl` → "conversation
  pruned"/"conversation only" note (FR-028). The Raw-log view shows the native `transcript.jsonl`.
- **Context** — a "context given to the agent" card at the top of the main column (prompt +
  instruction-file contents, collapsible) plus the configuration rail (harness/model/tools/MCP,
  container image + digest, mounts, env-var names, completeness statement) — the full `context.json`
  (FR-022, SC-008).
- **Report** — the spec-007 `report.json` fields (a Usage section in the rail) + `notes`/`error`.
- **Files** — the run's output artifacts in the rail, browsable with preview + download (FR-019a).
