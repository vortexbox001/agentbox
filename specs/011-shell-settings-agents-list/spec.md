...
2. **Given** an agent that is both an asset and a job, **When** its row renders, **Then** it renders as a grouped pair of sub-rows — Name/Harness/Model span both (rowspan), then one sub-row per nature — where the asset sub-row shows the asset Kind badge and its asset-schedule pills and the job sub-row shows the job Kind badge and its job-schedule pills; both sub-rows carry the same run data and are narrowed/filled as one unit.
...
5. **Given** the Light or Dark theme, **When** the foot Dagster status block or a schedule/sensor toggle's checked track renders, **Then** it uses the indigo accent ramp to signal the Dagster connection point, in both themes; other elements — including the Latest-run link, which uses the theme's own link accent (teal) even though it deep-links to Dagster — keep the theme's own accent.
...
- **FR-010**: Indigo MUST NOT be a selectable theme. Instead, an indigo accent colour ramp MUST be added to both the Light and Dark themes and applied to the primary Dagster connection points — the foot Dagster status block and the schedule/sensor toggles' checked track — so they read as indigo in every theme. Other elements keep the theme's own accent; in particular the per-agent Latest-run link, though it deep-links to Dagster, uses the theme's own link accent (teal) rather than indigo, and the Run-history bars keep their run-status colours.
