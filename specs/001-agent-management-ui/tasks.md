...
- [ ] T054 Map storage failures to a distinct error: catch `PermissionError`/`OSError` in `ui/agents_store.py` and `ui/prompts_store.py`, raise a `StorageError(operation, path)` that `ui/main.py` turns into `507 {"error": "storage", "message": "cannot <op> <dir>/<file>: <os error>"}` per contracts/http-api.md; add cases to `ui/tests/test_api.py` using a read-only `tmp_path` (chmod) and a missing, uncreatable prompts directory. (Belongs logically to Phase 2; numbered here to keep earlier IDs stable — do it before T052.)
