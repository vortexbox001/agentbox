...
- What happens when an agent YAML file has invalid syntax? The system displays the agent in the list with an error indicator and allows the user to open and fix it.
...
- **FR-003**: System MUST display an inline explanation for each form field describing what the parameter does, its valid values, and its default
...
- **FR-012**: System MUST present the app shell with a left-hand navigation sidebar (LHN), a top bar (TLN), and a main content pane (MWP). In this release the top bar carries the current page title, a breadcrumb (e.g., "Agents / my-agent"), and nothing else; top-level navigation items are reserved for later releases
...
- **FR-019**: Users MUST be able to delete an agent from its detail page; deletion MUST require an explicit confirmation, MUST remove only the agent's YAML file, and MUST NOT modify or remove the agent's workspace or output directories
...
- **SC-005**: New prompts created through the UI are immediately available for selection by other agents without page refresh
...
- **SC-008**: A person who has not used the UI before can complete the quickstart "Create" checklist without consulting the README, relying only on the inline explanations
